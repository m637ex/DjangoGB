...
urlpatterns = [
    ...
    path('hello/', hello, name='hello'), # - hello - функция
    path('hello2/', HelloView.as_view(), name='hello2'), # HelloView - класс, as_view - метод класса
    path('posts/<int:year>/', year_post, name='year_post'), # http://127.0.0.1:8000/les3/posts/2022/
    path('posts/<int:year>/<int:month>/', MonthPost.as_view(), name='month_post'), # http://127.0.0.1:8000/les3/posts/2022/6/
    path('posts/<int:year>/<int:month>/<slug:slug>/', post_detail, name='post_detail'), # http://127.0.0.1:8000/les3/posts/2022/6/python/
    path('if/', TemplIf.as_view(), name='templ_if'), # шаблон используя TemplateView
